...
- Repository-level backend tests live in `tests/`; frontend component and API tests live beside frontend source files as `*.test.ts(x)`.
...
- `tests/`: pytest coverage for backend routes, recommender, and advisor logic.
...
- Keep new tests compatible with the root npm scripts. `npm run test` must run all frontend and backend tests; `npm run check` must remain the broad pre-PR validation command.

## Testing Format

- Backend tests use pytest and live under `tests/` as `test_*.py`.
- Backend route-contract tests should use `fastapi.testclient.TestClient` against `backend.main:app`.
- Prefer monkeypatching `backend.services.modeling.advisor_pipeline` in route tests when the goal is API contract coverage rather than recommender/model scoring.
- Frontend tests use Vitest, React Testing Library, user-event, and jsdom.
- Place frontend tests next to the code they cover as `*.test.ts` or `*.test.tsx`, for example `frontend/src/features/draft-simulator/api.test.ts` and component-level `*.test.tsx` files.
- Mock `global.fetch` in frontend tests. Do not call the hosted Render backend or any external service from frontend tests.
- Use exact hero names in fixtures and assertions. Prefer small mock recommendation payloads that match the backend response shape.
- Add or update npm scripts when introducing a new test category so it is included in root `npm run test` or `npm run check`.
...
- The frontend test suite uses Vitest. For UI behavior changes, add or update component tests and then run `npm run test:frontend` or root `npm run test`.
...
- For API contract changes, update `tests/test_draft_api.py` and keep mocked route tests fast and deterministic.
...
- `tests/test_draft_api.py` is the first place to inspect before changing draft route request or response contracts.
